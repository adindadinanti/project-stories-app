  ...
  #setupForm() {
    ...
    this.#form.addEventListener('submit', async (event) => {
      ...
      if (this.#takenDocumentations.length === 0) {
        alert('Harap tambahkan minimal 1 foto');
        return;
      }
      ...
      if (!descriptionInput || !latitudeInput || !longitudeInput) {
        console.error('Required form elements not found');
        return;
      }

      const photoFile = this.#takenDocumentations[0].blob;
    ...
    const docInput = document.getElementById('documentations-input');
